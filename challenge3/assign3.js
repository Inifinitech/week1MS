...
//variable to allow user to enter benefits 
    const benefits = parseInt(document.getElementById("benefits").value);
    const totalDeductions = nhifdedct(grossSalary) + payededct(grossSalary) + nssf;
    const netSalary = (grossSalary + benefits + relief) - totalDeductions;
    document.getElementById("result").textContent = `KES ${netSalary.toFixed(2)}`;
    return netSalary;
}
